...
def setup_assume_role():
    ...
    os.environ['AWS_ACCESS_KEY_ID'] = os.environ["iam_auth_assume_aws_account"]
    os.environ['AWS_SECRET_ACCESS_KEY'] = os.environ["iam_auth_assume_aws_secret_access_key"]

    role_name = os.environ["iam_auth_assume_role_name"]
...
def setup_ecs():
    ...
    run_prune_command = f"{base_command} -v remote_gc_services --cluster {os.environ['iam_auth_ecs_cluster']}"
    run_test_command = f"{base_command} -d -v run_e2e_test --cluster {os.environ['iam_auth_ecs_cluster']} --task_definition {os.environ['iam_auth_ecs_task_definition']} --subnets {os.environ['iam_auth_ecs_subnet_a']} --subnets {os.environ['iam_auth_ecs_subnet_b']} --security_group {os.environ['iam_auth_ecs_security_group']} --files {mongo_binaries}/mongod:/root/mongod {mongo_binaries}/mongosh:/root/mongosh lib/ecs_hosted_test.js:/root/ecs_hosted_test.js {project_dir}:/root --script lib/ecs_hosted_test.sh"
    ...
    #  reason
    env = dict(AWS_ACCESS_KEY_ID=os.environ['iam_auth_ecs_account'],
               AWS_SECRET_ACCESS_KEY=os.environ['iam_auth_ecs_secret_access_key'])
...
def setup_regular():
    # Create the user.
    kwargs = dict(
        username=os.environ["iam_auth_ecs_account"],
        password=os.environ["iam_auth_ecs_secret_access_key"]
    )
    create_user(os.environ["iam_auth_ecs_account_arn"], kwargs)


def setup_web_identity():
    # Unassign the instance profile.
    env = dict(AWS_ACCESS_KEY_ID=os.environ["iam_auth_ec2_instance_account"],
               AWS_SECRET_ACCESS_KEY=os.environ["iam_auth_ec2_instance_secret_access_key"])
    ...
    env = dict(
        IDP_ISSUER=os.environ["iam_web_identity_issuer"],
        IDP_JWKS_URI=os.environ["iam_web_identity_jwks_uri"],
        IDP_RSA_KEY=os.environ["iam_web_identity_rsa_key"],
        AWS_WEB_IDENTITY_TOKEN_FILE=os.environ['iam_web_identity_token_file']
    )
    ...
    os.environ['AWS_WEB_IDENTITY_TOKEN_FILE'] = os.environ['iam_web_identity_token_file']
    os.environ['AWS_ROLE_ARN'] = os.environ["iam_auth_assume_web_role_name"]
