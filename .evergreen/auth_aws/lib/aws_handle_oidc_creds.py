...
def get_id_token(config=None, expires=None):
    ...
    client_id = config["client_id"]
    client_secret = config["client_secret"]
    response = provider.parse_authentication_request(
        f"response_type=code&client_id={client_id}&scope=openid&redirect_uri={MOCK_ENDPOINT}"
    )
    ...
    response = provider.handle_token_request(
        f"grant_type=authorization_code&subject_type=public&code={code}&redirect_uri={MOCK_ENDPOINT}",
        headers,
...
def get_user_id():
    ...
    return get_provider(config).authz_state.get_subject_identifier(
        "public", config["username"], "example.com"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        dest="command", help="The command to run (config, jwks, token, user_id)"
    )
