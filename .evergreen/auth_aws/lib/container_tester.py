...
def remote_ps_container(cluster):
    ...
    for task in task_list["tasks"]:
        ...
        enis = []
        for b in [
            a["details"] for a in task["attachments"] if a["type"] == "ElasticNetworkInterface"
        ]:
            for c in b:
...
def _remote_create_container_args(args):
    remote_create_container(
        args.cluster, args.task_definition, args.service, args.subnets, args.security_group
    )
...
def main() -> None:
    ...
    run_test_cmd.add_argument(
        "--endpoint", required=True, type=str, help="User and Host and port, ie user@host:port"
    )
    ...
    remote_create_cmd.add_argument(
        "--service", type=str, default=DEFAULT_SERVICE_NAME, help="ECS Service to create"
    )
    remote_create_cmd.add_argument(
        "--task_definition",
        type=str,
    ...
    remote_create_cmd.add_argument(
        "--subnets", type=str, nargs="*", default=ECS_DEFAULT_SUBNETS, help="EC2 subnets to use"
    )
    remote_create_cmd.add_argument(
        "--security_group",
        type=str,
