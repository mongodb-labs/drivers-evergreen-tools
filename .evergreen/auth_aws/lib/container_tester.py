...
ECS_DEFAULT_TASK_DEFINITION = (
    "arn:aws:ecs:us-east-2:579766882180:task-definition/tf-app:2"
)
...
def remote_ps_container(cluster):
    ...
    print(
        "Name                                       State    Public IP                Private IP               TaskDefinition  Health"
    )
    for task in task_list["tasks"]:
        ...
        public_ip = [n["Association"]["PublicIp"] for n in eni["NetworkInterfaces"]][0]  # noqa: RUF015
        ...
        print(
            f"{task_id:<43}{lastStatus:<9}{public_ip:<25}{private_ip_address:<25}{taskDefinition_short:<16}"
        )
...
def remote_create_container(
    cluster, task_definition, service_name, subnets, security_group
):
    """
...
def remote_gc_services_container(cluster):
    ...
    services_details = ecs_client.describe_services(
        cluster=cluster, services=services["serviceArns"]
    )

    not_expired_now = (
        datetime.datetime.now().astimezone() - DEFAULT_GARBAGE_COLLECTION_THRESHOLD
    )
    ...
    for service in services_details["services"]:
        ...
        if created_at < not_expired_now:
            print(
                "DELETING expired service %s which was created at %s."
                % (service["serviceName"], created_at)
            )
...
def main() -> None:
    ...
    sub = parser.add_subparsers(
        title="Container Tester subcommands", help="sub-command help"
    )
    ...
    run_test_cmd.add_argument(
        ...
        required=True,
        type=str,
        help="User and Host and port, ie user@host:port",
    )
    ...
    run_test_cmd.add_argument(
        ...
        type=str,
        nargs="*",
        help="Files to copy, each string must be a pair of src:dest joined by a colon",
    )
    run_test_cmd.set_defaults(func=_run_test_args)

    remote_ps_cmd = sub.add_parser("remote_ps", help="Stop Local Container")
    remote_ps_cmd.add_argument(
        "--cluster", type=str, default=ECS_DEFAULT_CLUSTER, help="ECS Cluster to target"
    )
    ...
    remote_create_cmd.add_argument(
        "--cluster", type=str, default=ECS_DEFAULT_CLUSTER, help="ECS Cluster to target"
    )
    remote_create_cmd.add_argument(
        "--service",
        ...
        help="ECS Service to create",
    )
    remote_create_cmd.add_argument(
        "--task_definition",
        type=str,
        default=ECS_DEFAULT_TASK_DEFINITION,
        help="ECS Task Definition to use to create service",
    )
    remote_create_cmd.add_argument(
        "--subnets",
        type=str,
        nargs="*",
        default=ECS_DEFAULT_SUBNETS,
        help="EC2 subnets to use",
    )
    remote_create_cmd.add_argument(
        "--security_group",
        type=str,
        default=ECS_DEFAULT_SECURITY_GROUP,
        help="EC2 security group use",
    )
    ...
    remote_stop_cmd.add_argument(
        "--cluster", type=str, default=ECS_DEFAULT_CLUSTER, help="ECS Cluster to target"
    )
    remote_stop_cmd.add_argument(
        "--service", type=str, default=DEFAULT_SERVICE_NAME, help="ECS Service to stop"
    )
    remote_stop_cmd.set_defaults(func=_remote_stop_container_args)

    remote_gc_services_cmd = sub.add_parser(
        "remote_gc_services", help="GC Remote Container"
    )
    remote_gc_services_cmd.add_argument(
        "--cluster", type=str, default=ECS_DEFAULT_CLUSTER, help="ECS Cluster to target"
    )
    remote_gc_services_cmd.set_defaults(func=_remote_gc_services_container_args)

    get_caller_identity_cmd = sub.add_parser(
        "get_caller_identity", help="Get the AWS IAM caller identity"
    )
    get_caller_identity_cmd.set_defaults(func=_get_caller_identity)

    remote_get_endpoint_cmd = sub.add_parser(
        "remote_get_endpoint", help="Get SSH remote endpoint"
    )
    remote_get_endpoint_cmd.add_argument(
        "--cluster", type=str, default=ECS_DEFAULT_CLUSTER, help="ECS Cluster to target"
    )
    remote_get_endpoint_cmd.add_argument(
        "--service", type=str, default=DEFAULT_SERVICE_NAME, help="ECS Service to stop"
    ...
    run_e2e_test_cmd.add_argument(
        "--script", required=True, type=str, help="script to run"
    )
    run_e2e_test_cmd.add_argument(
        "--files",
        type=str,
    ...
    print(
        "AWS_SHARED_CREDENTIALS_FILE: %s" % (os.getenv("AWS_SHARED_CREDENTIALS_FILE"))
    )
