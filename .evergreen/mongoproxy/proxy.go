...
func proxyMongoToClient(src net.Conn, dst net.Conn) {
	for {
		...
		// We found our instructions—apply them
		buf := raw
		sentAll := false
		...
		if len(buf) > 0 && !sentAll {
			dst.Write(buf)
		}

		// Stream the reset of the connection data if we have any left.
		if _, err := io.Copy(dst, src); err != nil {
			log.Printf("error copying remaining data: %v", err)
		}

		// In *all* cases half‐close the write side so the client sees EOF
