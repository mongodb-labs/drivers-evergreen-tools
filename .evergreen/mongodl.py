...
if TYPE_CHECKING:
    DownloadResult = NamedTuple("DownloadResult", [("is_changed", bool), ("path", Path)])
    ...
    DownloadableComponent = namedtuple(
        "DownloadableComponent", ["version", "target", "arch", "edition", "key", "data_json"]
    )
...
class Cache:
    ...
    def download_file(self, url: str) -> DownloadResult:
        ...
        """
        info = self._db(
            "SELECT etag, last_modified " "FROM mdl_http_downloads WHERE url=:url", url=url
        )
        ...
        try:
            ...
            assert dest.is_file(), ("The download cache is missing an expected file", dest)
...
def _dl_component(
    ...
    print(f"Download {component} {version}-{edition} for {target}-{arch}")
    if version == "latest-build":
        dl_url = _latest_build_url(target, arch, edition, component, latest_build_branch)
    else:
...
def main(argv: "Sequence[str]"):
    ...
    dl_grp.add_argument(
        "--out", "-o", help="The directory in which to download components. (Required)", type=Path
    )
    dl_grp.add_argument(
        "--version",
        "-V",
    ...
    dl_grp.add_argument(
        "--component",
        "-C",
        help="The component to download (Required). " 'Use "--list" to list available components.',
    )
    dl_grp.add_argument(
        "--only",
    ...
    dl_grp.add_argument(
        "--latest-build-branch",
        help="Specify the name of the branch to " 'download the with "--version=latest-build"',
        metavar="BRANCH_NAME",
    )
    ...
    if args.list:
        _print_list(cache.db, args.version, args.target, args.arch, args.edition, args.component)
        return None
