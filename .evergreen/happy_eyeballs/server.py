...
if platform.system() not in ["Darwin", "Windows"]:
    print(
        f"Only macOS (Darwin) and Windows are supported, but got: {platform.system()}",
        file=sys.stderr,
    )
    sys.exit(1)
...
parser.add_argument(
    "-c", "--control", default=10036, type=int, metavar="PORT", help="control port"
)
parser.add_argument(
    "--wait", action="store_true", help="wait for a running server to be ready"
)
...
async def control_server():
    ...
    srv = await asyncio.start_server(
        lambda reader, writer: on_control_connected(reader, writer, shutdown),
        "localhost",
        args.control,
    )
    print(
        f"{PREFIX}: listening for control connections on {args.control}",
        file=sys.stderr,
    )
    async with srv:
        await shutdown.wait()
...
async def on_control_connected(
    reader: asyncio.StreamReader, writer: asyncio.StreamWriter, shutdown: asyncio.Event
):
    ...
    on_ipv4_connected = lambda reader, writer: on_test_connected(  # noqa: E731
        "IPv4", writer, b"\x04", connected, slow
    )
    on_ipv6_connected = lambda reader, writer: on_test_connected(  # noqa: E731
        "IPv6", writer, b"\x06", connected, slow
    )
    # port 0: pick random unused port
    srv4 = await asyncio.start_server(
        on_ipv4_connected, "localhost", 0, family=socket.AF_INET, start_serving=False
    )
    srv6 = await asyncio.start_server(
        on_ipv6_connected, "localhost", 0, family=socket.AF_INET6, start_serving=False
    )
    ...
    test_tasks = [
        asyncio.create_task(
            test_listen("IPv4", srv4, data == b"\x04", connected, slow)
        ),
        asyncio.create_task(
            test_listen("IPv6", srv6, data == b"\x06", connected, slow)
        ),
    ]
    ...
    print(
        f"{PREFIX}: [slow {slow}] connection complete, test ports closed",
        file=sys.stderr,
    )
    print(f"{PREFIX}: ========================", file=sys.stderr)
...
async def wait_for_server():
    while True:
        try:
            control_r, control_w = await asyncio.open_connection(
                "localhost", args.control
            )
        except OSError as e:
    ...
    if data != b"\x01":
        ...
        sys.exit(1)
    print(
        f"{PREFIX}: happy eyeballs server ready on port {args.control}", file=sys.stderr
    )
