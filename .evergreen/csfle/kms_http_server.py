...
class AwsKmsHandler(kms_http_common.KmsHandlerBase):
    ...
    def _validate_signature(self, headers, raw_input):
        ...
        request = AWSRequest(
            method="POST", url="/", data=raw_input, headers=signed_headers_dict
        )
    ...
    def _do_encrypt(self, raw_input):
        ...
        if (
            kms_http_common.fault_type
            and kms_http_common.fault_type.startswith(kms_http_common.FAULT_ENCRYPT)
    ...
    def _do_encrypt_faults(self, raw_ciphertext):
        ...
        if kms_http_common.fault_type == kms_http_common.FAULT_ENCRYPT:
            self._send_reply(
                b"Internal Error of some sort.", http.HTTPStatus.INTERNAL_SERVER_ERROR
            )
            return
    ...
    def _do_decrypt(self, raw_input):
        ...
        if (
            kms_http_common.fault_type
            and kms_http_common.fault_type.startswith(kms_http_common.FAULT_DECRYPT)
            and not kms_http_common.disable_faults
        ):
...
def main():
    ...
    parser.add_argument(
        "--disable-faults", action="store_true", help="Disable faults on startup"
    )

    parser.add_argument("--ca_file", type=str, required=True, help="TLS CA PEM file")

    parser.add_argument(
        "--cert_file", type=str, required=True, help="TLS Server PEM file"
    )
    ...
    parser.add_argument(
        ...
        required=False,
        default=False,
        help="Require a client certificate in TLS connections",
    )
    ...
    kms_http_common.run(
        ...
        args.ca_file,
        AwsKmsHandler,
        cert_required=args.require_client_cert,
    )
