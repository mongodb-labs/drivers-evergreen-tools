...
def bootstrap(VERSION="latest", TOPOLOGY=None, **kwargs):
    if TOPOLOGY:
        return func("bootstrap mongo-orchestration", VERSION=VERSION, TOPOLOGY=TOPOLOGY, **kwargs)

    return func("bootstrap mongo-orchestration", VERSION=VERSION, **kwargs)
...
def targz_pack(target, source_dir, *include):
    return OD(
        [
            ...
            (
                "params",
                OD([("target", target), ("source_dir", source_dir), ("include", list(include))]),
            ),
        ]
    )
